...
class CartController {
    ...
    async saveProductToCart(req, res) {
        try {
            ...
            // Check if idCart is provided and if the cart exists
            let cart;
            if (idCart) {
                ...
                // Save product to the existing cart
                cart = await cartRepository.saveProductToCart(idProduct, idCart, quantity);
            } else {
                // Continue without handling the cart
                cart = await cartRepository.saveProductToCart(idProduct, null, quantity);
            }
    ...
    /* async saveProductToCart(req, res) {
        try {
            ...
            const cartExits = await cartRepository.getCart(idCart)
